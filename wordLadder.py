...
def ladderLength0(beginWord, endWord, wordDict):
    cal_dict = dict()
    is_distance_dict = dict()
    def is_distance_one(first, second):
        distance = 0
        if (first, second) in is_distance_dict:
            return is_distance_dict[(first, second)]
        for i, j in zip(first, second):
            if i != j:
                distance += 1
        if distance == 1:
            is_distance_dict[(first, second)] = True
            return True
        else:
            is_distance_dict[(first, second)] = False
            return False
    def cal_ladder_length(beginWord, endWord, length):
        r = []
        for word in wordDict:
            if word in cal_dict:
                continue
            if is_distance_one(beginWord, word):
                if is_distance_one(endWord, word):
                    return length + 1
                else:
                    r.append(word)
        l = []
        cal_dict[beginWord] = 1
        for word in r:
            l.append(cal_ladder_length(word, endWord, length + 1))
        l = [item for item in l if item != 0]
        if not l:
            return 0
        else:
            return min(l) + 1
    return cal_ladder_length(beginWord, endWord, 0)


def ladderLength1(beginWord, endWord, wordDict):
    ...
    def is_distance_one(first, second):
        ...
        if distance == 1:
            ...
            return False
    def cal_ladder_length(beginWord, endWord, wordDict):
        reached = []
        ...
        while endWord not in reached:
            to_add = set()
            to_remove = set()
            for reached_word in reached:
                for word in wordDict:
                    distance = 0
                    for i in x_range:
                        if reached_word[i] != word[i]:
                            distance += 1
                            if distance > 1:
                                break
                    if distance == 1:
            ...
            reached = to_add
        return distance
    return cal_ladder_length(beginWord, endWord, wordDict)
...
def ladderLength(beginWord, endWord, wordDict):
    ...
    wordDict.add(endWord)
    distance = 1
    chr_list = [chr(i) for i in xrange(ord('a'), ord('z')+1)]
    length = len(beginWord)
    while endWord not in reached:
        to_add = set()
        for reached_word in reached:
            for i in xrange(length):
                part1 = reached_word[:i]
                part2 = reached_word[i+1:]
                for ch in chr_list:
                    char = part1 + ch + part2
                    to_add.add(char)
        to_add = to_add & wordDict
        wordDict = wordDict - to_add
        distance += 1
        if not to_add:
            return 0
        reached = to_add
